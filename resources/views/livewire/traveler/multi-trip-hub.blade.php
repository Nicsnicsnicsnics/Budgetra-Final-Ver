<div style="display:flex;flex-direction:column;flex:1;">
    ...
    <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(320px,1fr));gap:20px;">
        ...
        <div class="card" style="overflow:hidden;border-radius:14px;">
            <div style="position:relative;height:210px;background:linear-gradient(135deg,#934B19,#C8874A);">
                @if ($trip->cover_image)
                <img src="{{ $trip->cover_image }}" style="width:100%;height:100%;object-fit:cover;display:block;" alt="{{ $trip->destination }}">
                @endif
                <span style="position:absolute;top:14px;left:14px;background:#FDF3EB;color:#934B19;font-size:11px;font-weight:700;padding:4px 12px;border-radius:20px;text-transform:uppercase;letter-spacing:.4px;">
                    {{ $tType }}
                </span>
            </div>
    ...
    <div style="position:fixed;inset:0;background:rgba(0,0,0,.5);z-index:2000;display:flex;align-items:center;justify-content:center;padding:20px;" wire:click.self="closeDetail">
        <div style="background:#fff;border-radius:20px;max-width:480px;width:100%;max-height:90vh;overflow-y:auto;padding:28px;">
            <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:4px;">
                ...
                <button wire:click="closeDetail" style="background:none;border:none;font-size:20px;cursor:pointer;color:#9CA3AF;line-height:1;">&times;</button>
            </div>
            <div class="text-muted" style="font-size:13px;margin-bottom:16px;">{{ $dt->start_date->format('M j') }} - {{ $dt->end_date->format('M j, Y') }}</div>

            <div style="height:170px;border-radius:14px;overflow:hidden;margin-bottom:18px;background:linear-gradient(135deg,#934B19,#C8874A);">
                @if ($dt->cover_image)
    ...
    <div style="position:fixed;inset:0;background:rgba(0,0,0,.5);z-index:2000;display:flex;align-items:center;justify-content:center;padding:20px;" wire:click.self="closeComparison">
        <div style="background:#fff;border-radius:20px;max-width:700px;width:100%;max-height:90vh;overflow-y:auto;padding:28px;">
